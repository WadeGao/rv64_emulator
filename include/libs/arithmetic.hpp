...
MulUnsignedHi(const T a, const T b) {
    ...
    const T kCarryBit = ((T)(type)a_x_b_mid + (T)(type)b_x_a_mid + (a_x_b_lo >> kTypeBits)) >> kTypeBits;

    const T kRes = a_x_b_hi + (a_x_b_mid >> kTypeBits) + (b_x_a_mid >> kTypeBits) + kCarryBit;
