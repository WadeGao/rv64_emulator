...
uint8_t GetShamt(const uint32_t inst_word, const bool is_rv32) {
  // rv64: shamt[5:0] = imm[5:0]
  // rv32: shamt[4:0] = imm[4:0]
  const uint32_t shamt_mask = is_rv32 ? kShamtImm32Mask : kShamtImm64Mask;
  const uint8_t shamt = static_cast<uint8_t>(
      GetImm(inst_word, RV64InstructionFormatType::kTypeI) & shamt_mask);
