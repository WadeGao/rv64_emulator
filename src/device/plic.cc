...
Plic::Plic(uint64_t cores, bool is_s_mode, uint64_t device_num)
    : contexts_(cores * (is_s_mode ? 2 : 1)), dev_num_(device_num + 1) {
  const uint8_t kCtxPerHart = is_s_mode ? 2 : 1;
  for (size_t i = 0; i < contexts_.size(); i++) {
    contexts_[i].id = i;
    contexts_[i].mmode = is_s_mode ? (i % kCtxPerHart == 0) : true;
  }
}

void Plic::UpdateExt(uint32_t src_id, bool fired) {
...
bool Plic::GetInterrupt(uint64_t ctx_id) {
...
bool Plic::Load(uint64_t addr, uint64_t bytes, uint8_t* buffer) {
  ...
  if (kPendingBase <= addr && addr < 0x1080) {
  ...
  if (kEnableBase <= addr && addr < 0x1f2000) {
...
bool Plic::Store(uint64_t addr, uint64_t bytes, const uint8_t* buffer) {
  if (kSourcePriorityBase <= addr && addr < kPendingBase) {
    const uint64_t kSourceIndex = addr >> 2;
    if (kSourceIndex > dev_num_ - 1) {
      return false;
