...
#ifdef DEBUG
    printf("dram init to all zeros, size %llu bytes\n", m_size);
#endif
}
...
uint64_t DRAM::Load(const uint64_t addr, const uint64_t bit_size) const {
    ...
    switch (bit_size) {
        ...
        default:
#ifdef DEBUG
            printf("dram invalid load from addr [%llu] bit_size [%llu], now abort\n", addr, bit_size);
#endif
            assert(false);
            break;
    }
...
void DRAM::Store(const uint64_t addr, const uint64_t bit_size, const uint64_t val) {
    switch (bit_size) {
        ...
        default:
#ifdef DEBUG
            printf("dram invalid store to addr [%llu] bit_size [%llu] val [%llu], now abort\n", addr, bit_size, val);
#endif
            assert(false);
            break;
    }
...
DRAM::~DRAM() {
#ifdef DEBUG
    printf("destroy a dram, size %llu bytes\n", m_size);
#endif
    m_size = 0;
}
